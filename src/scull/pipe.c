...
struct scull_pipe {
  ...
  char *rp, *wp;			/* where to read, here to write */
			    ...
			    loff_t *f_pos)
{
  ...
  /* ok, dta is there, return something */
  if(dev->wp > dev->rp)
 ...
 * error the semaphore will be released before running
 */
static int scull_getwritespace(struct scull_pipe *dev, struct file *filp)
{
  while(spacefree(dev) == 0) { /* full */
    ...
    /* prepare_to_wait_exclusive - sets 'exclusive' flag
     * in the wait queue entry and adds the process to the end
     * of the wait queue. Note that there is no way to perform
     * exclusive waits with wait_event and ts variants 
     */
 ...
 * (some are overlayed ith bare scull)
 */
