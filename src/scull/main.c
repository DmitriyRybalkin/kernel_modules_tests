...
MODULE_AUTHOR("Sample Linux Driver");
...
#ifdef SCULL_DEBUG_DEPRECATION
/* start and offset are ignored as small pages are passed only */
int scull_read_procmem(char *buf, char **start, off_t offset,
		       int count, int *eof, void *data)
{
  int i, j, len = 0;
  int limit = count - 80; /* Dont't print more than this */
...
{
  /* WARNING: /proc file generation is deprecated on KERNEL >= 3.10
   * so create_proc_read_entry would be undefined on new KERNEL version
   */
#ifdef SCULL_DEBUG_DEPRECATION
		 ...
		 unsigned int cmd, unsigned long arg)
{
  ...
  /* 
   * extract type and number bitfields, and don't decode
  ...
  switch(cmd) {
    ...
    default: /* reduntant, as cmd as checked against MAXNR */
      return -ENOTTY;
  }
		    ...
		    loff_t *f_pos)
{
  ...
  //if(down_interruptible(&dev->sem))	/* semaphore locker */
  if(mutex_lock_interruptible(&dev->mutex))
    return -ERESTARTSYS;
  if(*f_pos >= dev->size)
    goto out;
...
int scull_init_module(void)
{
  ...
  /*
   * Get range of minor numbers to work with, asking for a dynamic
...
/* Possible implementation of scullsingle
 * (permit a device to be opened by only one
 * process at a time (single opennes)
 *
 *
 ...
 * 	}
 *	// then, eveything else is copied from the bare scull device
 *	if((filp->f_flags & O_ACCMODE) == O_WRONLY)
 *		scull_trim(dev);
