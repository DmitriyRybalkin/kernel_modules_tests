...
static int scull_s_open(struct inode *inode, struct file *filp)
{
  ...
  /* then, everything else is opened from the bare scull device */
...
static int scull_u_open(struct inode *inode, struct file *filp)
{
  ...
  if(scull_u_count &&
    ...
    !capable(CAP_DAC_OVERRIDE)) { /* still allow root */
      ...
      return -EBUSY; /* -EPERM would conuse the user */
  }
...
static int scull_w_open(struct inode *inode, struct file *filp)
{
  ...
  while(!scull_w_available()) {
    ...
    if(wait_event_interruptible(scull_w_wait, scull_w_available()))
      return -ERESTARTSYS; /* tell tje fs layer to handle it */
    spin_lock(&scull_w_lock);
  }
...
static int scull_w_release(struct inode *inode, struct file *filp)
{
  ...
  if(temp == 0)
    wake_up_interruptible_sync(&scull_w_wait); /* awake other uids */
  return 0;
}
...
/* Cloning device on open (possible if the device is not bound to specific hw object )*/
...
/* The list of devices and lock to protect it */
...
static int scull_c_release(struct inode *inode, struct file *filp)
{
   ...
   * A 'real' cloned device should be freed on the last close
   */
  return 0;
}
...
/*
 * This is called by cleanup module or on failure
 * It is required to never fail, even if nothing was intitialized first
 */
